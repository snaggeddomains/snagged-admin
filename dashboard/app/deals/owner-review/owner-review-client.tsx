      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
        ...
        {data?.canMine && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button style={{ ...btnGood, whiteSpace: "nowrap" }} disabled={mining || remining} onClick={() => remineBulk("all", false)} title="Runs the miner on EVERY outstanding pending card in one go — batches through them all locally (zero Gmail) until none are left. Resumes where it left off; the count only goes down. This is the button to bulk-run pending.">{remining ? "Re-mining…" : `🔁 Re-mine all pending${data?.remine && Number.isFinite(data.remine.remaining) && data.remine.remaining > 0 ? ` (${data.remine.remaining})` : ""}`}</button>
            <button style={{ ...btn, whiteSpace: "nowrap" }} disabled={mining || remining} onClick={mine} title="Mine the acquisition thread for every Master Txn without a card yet — pulls the seller + full name automatically. Runs in batches; click again to continue.">{mining ? "Mining…" : "⛏ Mine backlog"}</button>
