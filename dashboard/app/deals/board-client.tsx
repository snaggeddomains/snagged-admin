...
import { STAGES, PRIORITIES, SOURCES } from "@/lib/deals/stages";
...
type Resp = { ok: boolean; configured: boolean; deals: Deal[]; stats: Stats | null; assignees: { email: string }[]; canSeeAll: boolean; me: string; error?: string };
...
export default function BoardClient() {
  ...
  return (
    <main style={{ maxWidth: 1400, margin: "0 auto", padding: "0 4px" }}>
        ...
        {STAGES.map((stage) => {
          ...
          return (
            <div key={stage}
              ...
              style={{ flex: "0 0 236px", background: over ? "#eef4f0" : "var(--paper-2,#f4f1ea)", borderRadius: 10, padding: 8, minHeight: 120, border: over ? "1.5px dashed var(--coral,#e2674a)" : "1.5px solid transparent" }}>
              ...
              {col.map((d) => (
                <div key={d.id}
                  ...
                  style={{ background: "#fff", border: "1px solid var(--line,#e6e0d3)", borderRadius: 8, padding: "9px 10px", marginBottom: 8, cursor: dragEnabled ? "grab" : "pointer", boxShadow: "0 1px 2px rgba(0,0,0,0.04)", opacity: dragId === d.id ? 0.5 : 1 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
                    <span style={{ fontSize: 11.5, color: "var(--muted,#889)" }}>{d.owner_email ? d.owner_email.split("@")[0] : "Inbox"}</span>
                    <span style={{ fontSize: 11.5, fontWeight: 600, color: "var(--navy-2,#4a5b66)" }}>{usd(d.asking_price || d.appraisal_value)}</span>
                  </div>
...
function NewDealModal({ assignees, onClose, onCreated }: { assignees: { email: string }[]; onClose: () => void; onCreated: (id: string) => void }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(20,25,30,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--paper,#fff)", borderRadius: 14, padding: 20, width: "min(440px,100%)", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <input style={input} value={f.budgetRange} onChange={(e) => set("budgetRange", e.target.value)} placeholder="e.g. $5k–$25k" />
        ...
        <select style={input} value={f.ownerEmail} onChange={(e) => set("ownerEmail", e.target.value)}>
          ...
          {assignees.map((a) => <option key={a.email} value={a.email}>{a.email}</option>)}
        </select>
